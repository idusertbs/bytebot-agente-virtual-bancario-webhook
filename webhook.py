                ...
                if debito[i]['nombre'] == debito_context:
                    ...
                    for j in range(0,len(cuentas_json)):
                        if cuentas_json[j]["alias"] == debito_sueldo:                            
                            ...
                            json_string = u'{ "type": 4, "platform": "facebook", "payload": { "facebook": { "attachment": { "type": "template", "payload": { "template_type": "button", "text": "'+ speech_saldo_1 + "\n" + speech_saldo_2  + "\n" + speech_saldo_3 +'","buttons": [{ "type": "postback", "title": "Generar Reporte", "payload": "Generar Grafica ' + str(debito[i]['nombre']) + " " + str(cuentas_tipo_saldo_nombres[0]) + '" },{"type": "postback", "title": "Consultar Movimientos", "payload": "Consultar Movimientos ' + str(debito[i]['nombre']) + " " + str(cuentas_tipo_saldo_nombres[0])  +'"}]}}}} }'
                ...
                if debito[i]['nombre'] == debito_context:
                    ...
                    for j in range(0,len(cuentas_json)):
                        if cuentas_json[j]["alias"] == debito_sueldo:
                            ...
                            cuentas_tipo_saldo_saldos_array.append(cuentas_tipo_saldo_saldos)
                            cuentas_tipo_saldo_monedas_array.append(cuentas_tipo_saldo_monedas)
                            url_final = 'http://181.177.228.114:5000/grafica/' + str(cuentas_tipo_saldo_movimientos_dias) +'/'+ str(cuentas_tipo_saldo_movimientos_monto) +'/' + cuentas_tipo_saldo_saldos +'/'+ str(documento) + '/Cuentas/' + debito + '/' + debito_sueldo +'/' + cuentas_tipo_saldo_monedas
